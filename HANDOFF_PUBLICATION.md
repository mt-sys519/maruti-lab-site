...
サイト本体とPromptTerm CLOCKのダウンロードページはローカルで完成確認中です。初回コミット `205590d` を作成済みです。GitHub remoteの設定とpushはまだ行っていません。
...
- インストーラーSHA-256一致
- GitHub連携によるmain push時の自動デプロイ成功
- Build認証をMaruti Lab専用トークンへ分離済み

## 次回の順序

1. トップを1100px前後、900px、390pxで最終目視確認
2. 不要な旧画像を参照状況を確認したうえで整理
3. GitHubに非公開または公開リポジトリを作り、remoteを設定してpush
4. 公開URLを決め、`NEXT_PUBLIC_SITE_URL`を設定
5. Cloudflare側のプロジェクトをGitHubへ接続
6. 本番で全ページ、全外部リンク、画像、ダウンロードを確認
7. ダウンロード計測を実装
8. 計測内容に合わせてPrivacyへ集計記録の説明を追記
