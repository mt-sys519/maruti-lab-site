...
export function mountHyperProp(root) {
  ...
  // what was the lake is the street far below, with shops lit across it and cars both
  // ways: come down there and the flight is over. The buildings in play rise from under
  // the picture, so nothing stands in water. The backdrop is drawn the way pixel artists
  // build night skylines: far towers nearly sky-coloured with varied crowns and a few
  // pinpricks, nearer ones darker with windows lit a floor at a time, light from the last
  // of the sunset on the left. The buildings in play are at the plane's scale (facade).
  const CT = {
  ...
  // ---- the buildings the plane flies among ----
  // They are at the plane's depth, so at its scale: the pilot is about 10px, a pixel about
  // 17cm, a storey about 20px, and a window the height of a person. Smaller windows would
  // make the pilot a giant several storeys tall.
  ...
  // The tower the run is on, reaching down out of the picture, with setbacks behind.
  const cliffCity = (() => {
  ...
  const HINTS = {
    run: { keys: 'LR', k: 1, jp: (t) => [L('助走をつけて走る', 'Run to build up speed'), t ? L('人差し指と中指で PEDAL を連打', 'Drum PEDAL with two fingers') : L('← → を連打', 'Drum ← →')] },
    ...
    seated: { keys: 'LR', k: 0, big: 'PEDAL!', jp: (t) => [L('乗り込んだ！ 漕いで加速', 'Aboard! Pedal to speed up'), t ? L('PEDAL を連打して漕ぐ', 'Drum PEDAL to turn the prop') : L('← → を連打して漕ぐ', 'Drum ← → to turn the prop')] },
  ...
  function updatePlate() {
    ...
    const [what, press] = {
      ...
      ready: [t ? L('人差し指と中指で PEDAL を連打', 'Drum PEDAL with two fingers') : L('Enter / Space でスタート', 'Enter / Space to start'),
        opened() > 1 ? (t ? L('▲ ▼ で面をえらぶ', '▲ ▼ to choose a stage') : L('↑ ↓ で面をえらぶ', '↑ ↓ to choose a stage')) : au.enabled ? '' : L('音は本体の上の SOUND を ON に', 'Turn SOUND on (top) to hear it')],
      run: [pedalTo('走る', 'run'), t ? L('赤い杭のあたりで ▲ で乗り込む', '▲ at the red stake to board') : L('赤い杭のあたりで ↑ で乗り込む', '↑ at the red stake to board')],
      board: ['', ''],
      roll: [pedalTo('漕ぐ', 'turn the prop'), t ? L('▲ で機首上げ', '▲ to lift the nose') : L('↑ で機首上げ', '↑ to lift the nose')],
      fly: s.stall ? [L('失速！', 'Stall!'), t ? L('▼ で機首を下げて速度を戻す', '▼ to drop the nose, regain speed') : L('↓ で機首を下げて速度を戻す', '↓ to drop the nose, regain speed')] : [pedalTo('漕ぐ', 'turn the prop'), t ? L('▲ ▼ で機首', '▲ ▼ to pitch') : L('↑ ↓ で機首', '↑ ↓ to pitch')],
