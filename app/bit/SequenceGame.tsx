...
const levelNotes: Record<Difficulty, string> = { beginner: "1規則 / 等間隔", intermediate: "2規則 / 交互", advanced: "差分変化 / 2系列" };
const emptyBest: Record<Difficulty, number> = { beginner: 0, intermediate: 0, advanced: 0 };
const initialProblem = tutorials.beginner;

function Sequence({ problem }: { problem: SequenceProblem }) {
  return (
    <div className="bitSequenceRow" role="img" aria-label={problem.spoken}>
      {problem.display.map((value, index) => (
        <span className={`bitSequenceTerm ${value === null ? "isUnknown" : ""}`} key={`${index}-${value ?? "unknown"}`}>
          {value ?? "?"}
        </span>
      ))}
    </div>
...
export function SequenceGame() {
  ...
  const [problem, setProblem] = useState<SequenceProblem>(initialProblem);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        const saved = JSON.parse(localStorage.getItem("marutibit:sequence:best") || "null");
  ...
  useEffect(() => {
    ...
    void createResultCard({
      series: "MarutiBit", gameNumber: "GAME 003", gameTitle: "SEQUENCE",
      gameDescription: "数列の規則を見抜くゲーム", questions: TOTAL_QUESTIONS,
      level: levelNames[difficulty], score, correct: `${correct} / ${TOTAL_QUESTIONS}`,
      time: `${(totalTime / 1000).toFixed(1)}s`, url: "https://marutilab.com/bit/sequence",
    }).then((file) => { if (!cancelled) setShareCard(file); });
    return () => { cancelled = true; };
  }, [correct, difficulty, phase, score, totalTime]);

  const begin = (level = difficulty) => {
    const historyKey = `marutibit:sequence:recent:${level}`;
    ...
    const session = createUniqueQuestionSession({
      ...
      create: (index) => makeSequenceProblem(level, index),
      signature: sequenceProblemSignature,
    });
  ...
  const next = () => {
    ...
    if (question === TOTAL_QUESTIONS - 1) {
      ...
      setBest(updated);
      try { localStorage.setItem("marutibit:sequence:best", JSON.stringify(updated)); } catch { /* Local storage is optional. */ }
      setPhase("complete");
      return;
    }
  ...
  const share = async () => {
    playTap("action");
    const title = "MarutiBit「SEQUENCE」";
    const text = `${title}\n数列の規則を見抜く${TOTAL_QUESTIONS}問チャレンジ｜${levelNames[difficulty]}\nSCORE ${score} / ${correct} CORRECT / TIME ${(totalTime / 1000).toFixed(1)}s`;
    const url = "https://marutilab.com/bit/sequence";
    ...
    return (
      <section className="bitGameShell" aria-label="SEQUENCEゲーム">
        ...
        <div className="bitGameSummary">
          ...
          <div><strong>数の並びから「？」を求める。</strong><p>等間隔の並びから、2つの規則が交互に現れる数列と2系列の交差へ進む全5問です。</p></div>
        </div>
        ...
        <div className="bitPakuShareRow">
          <ShareButton title="MarutiBit「SEQUENCE」" text="数の並びに隠れた規則を見抜く順番推理ゲーム" url="https://marutilab.com/bit/sequence" />
          <XShareButton variant="compact" text={"MarutiBit「SEQUENCE」\n数の並びに隠れた規則を見抜く順番推理ゲーム"} url="https://marutilab.com/bit/sequence" />
        </div>
      </section>
    );
  }
  ...
  return (
    <section ref={gameShellRef} className="bitGameShell" aria-label="SEQUENCEゲーム">
      {pauseOverlay}
      ...
      <div className="bitSequenceBoard"><Sequence problem={problem} /></div>
      {phase === "answered" ? (
